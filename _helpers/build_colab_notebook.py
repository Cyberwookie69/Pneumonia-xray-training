...
NOTEBOOK_BUILD_DATE = "2026-05-11"
...
CELLS = [
    md(
        f"# Pneumonia detection — CNN design study\n"
        f"\n"
        f"**Notebook version `{NOTEBOOK_VERSION}` · built {NOTEBOOK_BUILD_DATE}**  \n"
        f"Generated by `_helpers/build_colab_notebook.py`. Do not edit cells by hand — "
        f"changes are overwritten on the next rebuild.\n"
        f"\n"
    ...
    code(
        f"print('Pneumonia notebook v{NOTEBOOK_VERSION} (built {NOTEBOOK_BUILD_DATE})')\n"
        f"!nvidia-smi --query-gpu=name,memory.total --format=csv,noheader\n"
        f"%cd /content\n"
        f"!git clone https://github.com/Cyberwookie69/Pneumonia-xray-training.git\n"
        f"%cd /content/Pneumonia-xray-training"
    ),
    md(
    ...
    code(
        ...
        "else:\n"
        "    print('⚠ No HF_TOKEN in Colab Secrets — section 16 (RAD-DINO) will fall '\n"
    ...
    md(
        ...
        "\n"
        "Holds the A3 winner platform (BN + Dropout + WD + light augmentation). One-by-one "
        "additive tests of training-time techniques that attack the *remaining* failure modes — "
        "sigmoid saturation (label smoothing), optimiser noise (SWA), augmentation strength "
        "(TrivialAugmentWide), spatial regularisation (CutMix), and the Lion optimiser as an "
        "AdamW alternative. Each row is a single-fold single-seed run; the goal is to populate "
        "the report appendix with **calculations for the tried paths**, not to crown a new "
        "winner from this table alone. Variants that beat the noise floor (≥ 0.03 pp) are then "
        "stacked into the §10 champion config."
    ),
    code(
    ...
    code(
        ...
        "\n"
        "# Which model's attention to visualise:\n"
        "#   'a3_combo'      — the A3 winner (compact attention, the 'classic' look)\n"
        "#   'champion_f0'   — the stacked champion (smoothing+CutMix+SWA; more diffuse,\n"
        "#                     since SWA averages weights and CutMix trains the model to\n"
        "#                     look at local patches rather than one global feature)\n"
        "GRADCAM_RUN = 'a3_combo' if os.path.exists(\n"
        "    f'{RUNS_ROOT}/a3_combo/best_state.pt') else 'champion_f0'\n"
        "print(f'Grad-CAM source: {GRADCAM_RUN}')\n"
        "\n"
        "device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')\n"
        "\n"
        "summ = json.load(open(f'{RUNS_ROOT}/{GRADCAM_RUN}/summary.json'))\n"
        ...
        "model.load_state_dict(torch.load(f'{RUNS_ROOT}/{GRADCAM_RUN}/best_state.pt',\n"
        "                                  map_location=device))\n"
        "model.eval()\n"
        "\n"
    ...
    md(
        "---\n"
        "## 14. Transfer-learning comparison (ResNet50 + ImageNet)\n"
        "\n"
    ...
    md(
        "---\n"
        "## 15. Transfer learning — BiomedCLIP (vision-language, biomedical literature pretrained)\n"
        "\n"
        ...
        "leakage risk (see §16), but worth noting."
    ),
    code(
    ...
    md(
        ...
        "\n"
        "**Important**: Nano Banana (`gemini-2.5-flash-image`) is **not available on the "
        "Gemini free tier** — the call returns 429 / RESOURCE_EXHAUSTED with a hard "
        "quota limit of 0. This is normal and expected; the cell handles the 429 "
        "gracefully and the static fallback below renders the same diagram. Skip this "
        "cell entirely if you're on the free tier.\n"
        "\n"
        "*Get a key at https://aistudio.google.com/app/apikey → add via the 🔑 "
    ...
    code(
        ...
        "            print('⚠ Nano Banana (gemini-2.5-flash-image) is not available on '\n"
        "                  'the Gemini free tier (quota limit: 0).')\n"
        "            print('   This is normal — the static matplotlib fallback in the '\n"
        "                  'next cell renders the same diagram from the repo.')\n"
        "        elif 'UNAVAILABLE' in msg or '503' in msg:\n"
        "            print('⚠ Nano Banana is temporarily overloaded (503). '\n"
        "                  'Re-run the cell in a few minutes, or use the static fallback.')\n"
        "        else:\n"
        "            print(f'⚠ Nano Banana request failed: {msg[:200]}')\n"
        "            print('   Static fallback in the next cell still works.')\n"
        "        response = None\n"
        "\n"
        "    saved = False\n"
    ...
    code(
        ...
        "                print('⚠ Gemini free-tier quota hit. Retry in 1 min '\n"
        "                      'or use a paid key.')\n"
        "            elif '503' in msg or 'UNAVAILABLE' in msg:\n"
        "                print('⚠ Gemini temporarily overloaded (503). Retry in 1-3 min.')\n"
        "            else:\n"
        "                print(f'⚠ Gemini call failed: {msg[:200]}')\n"
        "            response = None\n"
        "\n"
        "        if response is None:\n"
        "            print('(no response — skipping code generation)')\n"
        "        else:\n"
    ...
    md(
        "## 20. A2 — Stride / padding / activation\n"
        "\n"
    ...
    md(
        "## 22. Mixup / CutMix / Manifold Mixup demo\n"
        "\n"
    ...
    md(
        "## 23. Champion ensemble — medical KPIs\n"
        "\n"
    ...
    md(
        "## 24. Transfer-learning baselines — medical KPIs\n"
        "\n"
        ...
        "\n"
        "BiomedCLIP and RAD-DINO numbers carry the leakage caveats from §15 and §16."
    ),
    code(
    ...
    md(
        "## 25. Headline comparison — 4 KPIs across approaches\n"
        "\n"
        "Stacked bullet charts, one row per medical KPI. Dot **colour** signals "
        "clinical-zone status (not approach identity — that lives in the text label):\n"
        "\n"
        ...
        "\n"
        "Best-accuracy threshold is used for sensitivity and specificity. Per-metric "
        "thresholds are sourced from Appendix A (ceiling) and Appendix B (usability)."
    ),
    code(
        "approaches = [\n"
    ...
    md(
        ...
        "\n"
        "Sends the just-generated headline KPI image to Gemini 2.5 Flash "
        "(vision-enabled, free tier) for a 300-word critique. Useful catch "
        "for missing labels, accessibility issues, or visualisation choices "
        "that may mislead a reader. Requires `GEMINI_API_KEY` in Colab Secrets "
        "(same one used for §17 Nano Banana)."
    ),
    code(
        "import os\n"
        "try:\n"
        ...
        "        msg = str(e)\n"
        "        if '429' in msg or 'RESOURCE_EXHAUSTED' in msg:\n"
        "            print('⚠ Gemini free-tier quota hit. Wait a minute and retry, '\n"
        "                  'or use a paid key.')\n"
        "        elif '503' in msg or 'UNAVAILABLE' in msg:\n"
        "            print('⚠ Gemini temporarily overloaded (503). Retry in 1-3 min.')\n"
        "        else:\n"
    ...
    md(
        "## 26. Visual report dashboard\n"
        "\n"
        "Four eye-candy visualisations of the headline result, each reading the same\n"
        "cached `test_probs.npy` + `test_labels.npy` + `medical_kpis.json` artefacts —\n"
        "no extra GPU time, just rendering on top of finished runs.\n"
        "\n"
        "1. **Scorecard per approach** — four KPIs with clinical-zone backgrounds (red /\n"
        "   green / grey) and ±1.96σ error bars from the binomial noise floor.\n"
        "2. **ROC overlay** — all approaches on one chart, zoomed to the top-left\n"
        "   clinical region, sweet-zone box highlighted.\n"
        "3. **Confusion-matrix grid** — heatmap per approach at the val-tuned operating\n"
        "   point, raw counts annotated.\n"
        "4. **Reliability diagrams** — ECE made visual: per-bin bars vs perfect-\n"
        "   calibration diagonal.\n"
        "\n"
        "Cells degrade gracefully — anything whose `medical_kpis.json` is missing is\n"
        "skipped with a note instead of crashing."
    ),
    code(
        "# 26.1 Scorecard per approach — Sens / Spec / AUROC / 1-ECE with min/max bands.\n"
        "import json, os, numpy as np, matplotlib.pyplot as plt\n"
        "\n"
        "APPROACHES = [\n"
        "    ('Custom CNN\\n(from scratch)', 'champion_ensemble'),\n"
        "    ('ResNet50 + IN',                'ensemble'),\n"
        "    ('BiomedCLIP',                   'biomedclip_ensemble'),\n"
        "    ('RAD-DINO',                     'rad_dino_ensemble'),\n"
        "]\n"
        "# (label, kpi-key, min, max, inverted)\n"
        "KPIS = [('Sensitivity', 'sensitivity', 0.95, 0.97, False),\n"
        "        ('Specificity', 'specificity', 0.90, 0.95, False),\n"
        "        ('AUROC',       'auroc',       0.95, 0.98, False),\n"
        "        ('1 - ECE',     'ece',         0.90, 0.95, True)]\n"
        "BAD, GOOD, SUS = '#FFCDD2', '#C8E6C9', '#E0E0E0'\n"
        "\n"
        "loaded = []\n"
        "for label, run in APPROACHES:\n"
        "    p = f'{RUNS_ROOT}/{run}/medical_kpis.json'\n"
        "    if os.path.exists(p):\n"
        "        loaded.append((label, json.load(open(p))))\n"
        "    else:\n"
        "        print(f'(skip {run} — no medical_kpis.json)')\n"
        "if not loaded:\n"
        "    print('No data yet — run the training cells first.')\n"
        "else:\n"
        "    fig, axes = plt.subplots(1, len(loaded), figsize=(4.5*len(loaded), 6),\n"
        "                              sharey=True)\n"
        "    if len(loaded) == 1: axes = [axes]\n"
        "    for ax, (label, k) in zip(axes, loaded):\n"
        "        op = k['operating_points'].get('best_accuracy',\n"
        "             list(k['operating_points'].values())[0])\n"
        "        n = k['n_test']\n"
        "        values = {'sensitivity': op['sensitivity'],\n"
        "                  'specificity': op['specificity'],\n"
        "                  'auroc':        k['auroc'],\n"
        "                  'ece':          1.0 - k['ece']}\n"
        "        for row, (name, key, lo, hi, inv) in enumerate(KPIS):\n"
        "            v = values[key]\n"
        "            sigma = (v * (1 - v) / n) ** 0.5\n"
        "            y = len(KPIS) - 1 - row\n"
        "            ax.axhspan(y - 0.4, y + 0.4, xmin=0,   xmax=(lo-0.75)/0.25, color=BAD)\n"
        "            ax.axhspan(y - 0.4, y + 0.4, xmin=(lo-0.75)/0.25,\n"
        "                       xmax=(hi-0.75)/0.25, color=GOOD)\n"
        "            ax.axhspan(y - 0.4, y + 0.4, xmin=(hi-0.75)/0.25, xmax=1, color=SUS)\n"
        "            ax.errorbar(v, y, xerr=1.96*sigma, fmt='o', color='black',\n"
        "                        markersize=10, capsize=4, zorder=5)\n"
        "            status = '✓ PASS' if v >= lo and v <= hi else ('▲ HIGH' if v > hi\n"
        "                     else '✗ FAIL')\n"
        "            colr = '#2E7D32' if status == '✓ PASS' else ('#757575' if 'HIGH' in\n"
        "                     status else '#C62828')\n"
        "            ax.text(1.01, y, f'  {v:.3f}\\n  {status}', va='center',\n"
        "                    fontsize=9, color=colr, fontweight='bold',\n"
        "                    transform=ax.get_yaxis_transform())\n"
        "        ax.set_yticks(range(len(KPIS)))\n"
        "        ax.set_yticklabels([n for n, *_ in KPIS][::-1])\n"
        "        ax.set_xlim(0.75, 1.0); ax.set_xticks(np.arange(0.75, 1.01, 0.05))\n"
        "        ax.set_title(label, fontsize=11, fontweight='bold', pad=10)\n"
        "        ax.grid(axis='x', alpha=0.25)\n"
        "    fig.suptitle('Scorecard — four medical KPIs vs clinical-zone thresholds\\n'\n"
        "                 '(green=sweet zone, red=unusable, grey=suspect; bars are 95% CI)',\n"
        "                 fontsize=12, y=1.02)\n"
        "    plt.tight_layout()\n"
        "    plt.savefig(f'{RUNS_ROOT}/dashboard_scorecard.png', dpi=110,\n"
        "                bbox_inches='tight', facecolor='white')\n"
        "    plt.show()"
    ),
    code(
        "# 26.2 ROC overlay — all four approaches in the top-left clinical zone.\n"
        "from sklearn.metrics import roc_curve, roc_auc_score\n"
        "\n"
        "fig, ax = plt.subplots(figsize=(8.5, 8))\n"
        "colors = ['#1976D2', '#F57C00', '#7B1FA2', '#388E3C']\n"
        "ax.axhspan(0.95, 0.97, xmin=0, xmax=0.10, facecolor='#C8E6C9', alpha=0.5,\n"
        "           label='Clinical sweet zone\\n(sens 0.95–0.97 × spec 0.90–0.95)')\n"
        "have = False\n"
        "for color, (label, run) in zip(colors, APPROACHES):\n"
        "    p_probs = f'{RUNS_ROOT}/{run}/test_probs.npy'\n"
        "    p_labs  = f'{RUNS_ROOT}/{run}/test_labels.npy'\n"
        "    if not (os.path.exists(p_probs) and os.path.exists(p_labs)):\n"
        "        continue\n"
        "    have = True\n"
        "    probs  = np.load(p_probs)\n"
        "    labels = np.load(p_labs).astype(int)\n"
        "    fpr, tpr, _ = roc_curve(labels, probs)\n"
        "    auc = roc_auc_score(labels, probs)\n"
        "    ax.plot(fpr, tpr, color=color, linewidth=2.4,\n"
        "            label=f'{label.split(chr(10))[0]} (AUROC={auc:.3f})')\n"
        "    # mark the val-tuned operating point\n"
        "    op = json.load(open(f'{RUNS_ROOT}/{run}/medical_kpis.json'))\n"
        "    op = op['operating_points'].get('best_accuracy')\n"
        "    if op:\n"
        "        ax.scatter(1-op['specificity'], op['sensitivity'], color=color,\n"
        "                   s=130, edgecolor='black', linewidth=1.2, zorder=5)\n"
        "ax.plot([0, 1], [0, 1], 'k--', alpha=0.3, linewidth=1)\n"
        "ax.set_xlabel('False positive rate (1 - specificity)'); ax.set_ylabel('Sensitivity')\n"
        "ax.set_title('ROC curves — zoomed to top-left clinical region\\n'\n"
        "             '(dot = val-tuned operating point)', fontsize=12)\n"
        "ax.set_xlim(0, 0.40); ax.set_ylim(0.85, 1.005)\n"
        "ax.legend(loc='lower right', fontsize=9); ax.grid(alpha=0.25)\n"
        "if have:\n"
        "    plt.tight_layout()\n"
        "    plt.savefig(f'{RUNS_ROOT}/dashboard_roc.png', dpi=110,\n"
        "                bbox_inches='tight', facecolor='white')\n"
        "    plt.show()\n"
        "else:\n"
        "    print('No test_probs.npy available yet.')"
    ),
    code(
        "# 26.3 Confusion matrices — one heatmap per approach at val-tuned threshold.\n"
        "panels = []\n"
        "for label, run in APPROACHES:\n"
        "    p_probs = f'{RUNS_ROOT}/{run}/test_probs.npy'\n"
        "    p_labs  = f'{RUNS_ROOT}/{run}/test_labels.npy'\n"
        "    p_kpi   = f'{RUNS_ROOT}/{run}/medical_kpis.json'\n"
        "    if not all(os.path.exists(x) for x in (p_probs, p_labs, p_kpi)):\n"
        "        continue\n"
        "    probs = np.load(p_probs); labs = np.load(p_labs).astype(int)\n"
        "    op = json.load(open(p_kpi))['operating_points'].get('best_accuracy')\n"
        "    t = op['threshold'] if op else 0.5\n"
        "    preds = (probs >= t).astype(int)\n"
        "    cm = np.array([[((preds == 0) & (labs == 0)).sum(),\n"
        "                    ((preds == 1) & (labs == 0)).sum()],\n"
        "                   [((preds == 0) & (labs == 1)).sum(),\n"
        "                    ((preds == 1) & (labs == 1)).sum()]])\n"
        "    panels.append((label, cm, t))\n"
        "if not panels:\n"
        "    print('No data — run training cells first.')\n"
        "else:\n"
        "    n = len(panels)\n"
        "    cols = min(n, 2); rows = (n + cols - 1) // cols\n"
        "    fig, axes = plt.subplots(rows, cols, figsize=(5.5*cols, 5*rows))\n"
        "    axes = np.atleast_2d(axes).flatten()\n"
        "    for ax, (label, cm, t) in zip(axes, panels):\n"
        ...
        "        ax.axis('off')\n"
        "    fig.suptitle('Confusion matrices @ val-tuned threshold',\n"
        "                 fontsize=13, y=1.00)\n"
        "    plt.tight_layout()\n"
        "    plt.savefig(f'{RUNS_ROOT}/dashboard_confusion.png', dpi=110,\n"
        "                bbox_inches='tight', facecolor='white')\n"
        "    plt.show()"
    ),
    code(
        "# 26.4 Reliability diagrams — ECE made visual, one per approach.\n"
        "panels = []\n"
        "for label, run in APPROACHES:\n"
        "    p_kpi = f'{RUNS_ROOT}/{run}/medical_kpis.json'\n"
        "    if not os.path.exists(p_kpi):\n"
        "        continue\n"
        "    k = json.load(open(p_kpi))\n"
        "    panels.append((label, k))\n"
        "if not panels:\n"
        "    print('No medical_kpis.json available.')\n"
        "else:\n"
        "    n = len(panels)\n"
        "    cols = min(n, 2); rows = (n + cols - 1) // cols\n"
        ...
        "        bins  = k['calibration_bins_uniform']\n"
        "        ece   = k['ece']\n"
        ...
        "        # bar height = bin accuracy; colour intensity = bin count share\n"
        "        for c, a, w, n_in in zip(centers, accs, widths, counts):\n"
        "            alpha = 0.25 + 0.65 * (n_in / max(counts) if max(counts) else 0)\n"
        "            ax.bar(c, a, width=w, color='#1976D2', alpha=alpha,\n"
        "                   edgecolor='black', linewidth=0.5)\n"
        "        ax.plot([0.5, 1.0], [0.5, 1.0], 'k--', alpha=0.5,\n"
        "                label='perfect calibration')\n"
        "        ax.set_xlim(0.5, 1.0); ax.set_ylim(0, 1.05)\n"
        "        ax.set_xlabel('predicted confidence'); ax.set_ylabel('observed accuracy')\n"
        "        ax.set_title(f'{label.replace(chr(10), \" \")}  ECE = {ece:.3f}',\n"
        "                     fontsize=10)\n"
        "        ax.legend(loc='upper left', fontsize=8); ax.grid(alpha=0.25)\n"
        "    for ax in axes[len(panels):]:\n"
        "        ax.axis('off')\n"
        "    fig.suptitle('Reliability diagrams — bar opacity = bin share of samples',\n"
        "                 fontsize=13, y=1.00)\n"
        "    plt.tight_layout()\n"
        "    plt.savefig(f'{RUNS_ROOT}/dashboard_reliability.png', dpi=110,\n"
        "                bbox_inches='tight', facecolor='white')\n"
        "    plt.show()"
    ),
    md(
        ...
        "in §15-§16 cell descriptions and the methodology flow diagram (Stage 3).\n"
        "\n"
    ...
    md(
        ...
        "\n"
        "Reads the actual KPIs from every `medical_kpis.json` on disk and asks "
        "Gemini 2.5 Flash to draft a 200-word academic conclusion paragraph. "
        "Useful as a starting point for the report-writing phase — review and "
        "edit before using. Requires `GEMINI_API_KEY` in Colab Secrets."
    ),
    code(
        "import os, json\n"
        "try:\n"
