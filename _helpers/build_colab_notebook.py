...
CELLS = [
    md(
        ...
        "- Setup + dataset: ~3 min\n"
        "- Smoke test: ~2 min\n"
        "- A1 depth ablation (4 rows): ~25 min\n"
        "- A2 stride/padding/activation (6 rows): ~40 min\n"
        "- A3 overfitting (6 rows): ~40 min\n"
        "- Champion 5-fold + KPIs + plots: ~45 min\n"
        "- Mixup demo + transfer-learning bonus: ~30 min\n"
        "- **Total: ~3 hours on a free T4** (well within the 12 h session limit)\n"
        "\n"
    ...
    code(
        ...
        "    rows.append((n, s['architecture']['n_params'], s['best_val_acc'],\n"
        "                 s['test_acc'], s['training']['elapsed_min']))\n"
        "\n"
        "print(f'{\"n_blocks\":>9}{\"params\":>12}{\"val_acc\":>10}{\"test_acc\":>10}{\"min\":>8}')\n"
        "for n, p, va, ta, m in rows:\n"
        "    print(f'{n:>9}{p:>12,}{va:>10.4f}{ta:>10.4f}{m:>8.1f}')"
    ),
    md(
        "---\n"
